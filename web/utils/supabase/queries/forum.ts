...
export const getForum = async (supabase: SupabaseClient, forumId: string): Promise<z.infer<typeof ForumSchema>> => {
  const { data, error } = await supabase.from("forum").select("*").eq("id", forumId).single();
...
export const deleteForum = async (supabase: SupabaseClient, forumId: string): Promise<void> => {
